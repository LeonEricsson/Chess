...
/**
 * The Bishop class handles everything
 * to do with the chess games Bishop
...
public class Bishop extends IterativeMove {
    private List<Point> bishopMoves = new ArrayList<>();

    public Bishop(Team pieceTeam, boolean hasMoved) {
            super(pieceTeam, hasMoved);
        }

    private final static int[] ROW_MOVES = {-1, -1, 1, 1};
    private final static int[] COL_MOVES = {-1, 1, -1, 1};
    ...
    @Override public List<Point> possibleMoves() {
        return getMoves(POSSIBLE_TILE_POS, ROW_MOVES, COL_MOVES, bishopMoves, ROW_MOVES.length);
    }
